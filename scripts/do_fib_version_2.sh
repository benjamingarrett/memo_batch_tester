...
function batch_run {
  ...
  while [ $min_size -le $max_size ] ; do
    python3 fib.py 1 $cache_size $min_size $version solve_once $folder $strategy $nru_k $nru_d
    min_size=$(( min_size+1 ))
  done
  outfile=$parent/$experiment.csv
  echo $outfile
  python3 ../../table_maker100/merge_on_sort_by_field.py $folder $outfile $experiment 16
  python3 ../../table_maker100/plot_folder.py $parent $experiment 'cache size' 'cache misses'
  end_time=$(date '+%Y_%m_%d__%H_%M_%S')
  echo start: $start_time  end: $end_time > $parent/report
}

batch_run 2ar nru_d_drunken_absolute 16 16 5 1 1000
