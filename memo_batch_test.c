...
int64_t characteristic_cache_size_linear_search(int64_t max_cache_size, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"starting linear search with cache size %ld\n", max_cache_size);fclose(fp);
...
int64_t characteristic_cache_size_binary_search(int64_t max_cache_size, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"Starting characteristic_cache_size_binary_search\n");fclose(fp);
  reset_lru_queue(max_cache_size);
  reset_problem();
  set_preemptive_halt_(0);
  solve_problem();
  ...
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"binary search, size %ld  misses %ld\n", max_cache_size, max_cache_misses);fclose(fp);
  ...
  while(min_cache_size < max_cache_size - 1){
    ...
    solve_problem();
    cache_misses = get_cache_misses_();
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"binary search, size %ld  misses %ld  min %ld  max %ld\n", 
...
int64_t non_preemptive_linear_search(int64_t max_cache_size, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"starting non-preemptive linear search with cache size %ld\n", max_cache_size);fclose(fp);
  reset_lru_queue(max_cache_size);
  reset_problem();
  set_preemptive_halt_(0);
  solve_problem();
...
int64_t solve_once(int64_t cache_size, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"starting solve_once with cache size %ld\n", cache_size);fclose(fp);
  reset_lru_queue(cache_size);
  reset_problem();
  set_preemptive_halt_(0);
  solve_problem();
...
int64_t ratio_based_cutoff(int64_t max_cache_size, double cutoff_ratio, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"starting ratio based cutoff\n");fclose(fp);
  reset_lru_queue(max_cache_size);
  reset_problem();
  set_preemptive_halt_(0);
  solve_problem();
  ...
  fclose(fp);
  set_cache_miss_threshold_(prev_cache_misses);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"linear search, size %ld  misses %ld\n", max_cache_size, prev_cache_misses);fclose(fp);
  int64_t cache_size = max_cache_size - 1;
  reset_problem();
  reset_lru_queue(cache_size);
  solve_problem();
  ...
  while(cache_size >= 2){
    ...
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"ratio = %f  cutoff = %f\n", ratio, cutoff_ratio);fclose(fp);
...
int64_t exact_cutoff(int64_t max_cache_size, int64_t min_cache_size, int64_t max_cache_misses, char cache_misses_out_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"starting exact cutoff\n");fclose(fp);
  reset_lru_queue(max_cache_size);
  reset_problem();
  set_preemptive_halt_(0);
  solve_problem();
  ...
  fclose(fp);
  set_cache_miss_threshold_(prev_cache_misses);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"exact_cutoff, min size %ld size %ld misses %ld\n", min_cache_size, max_cache_size, prev_cache_misses);fclose(fp);
  int64_t cache_size = max_cache_size - 1;
  reset_problem();
  reset_lru_queue(cache_size);
  solve_problem();
  ...
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"last cache size %ld\n", cache_size);fclose(fp);
  return -1;
}
...
int64_t exact_cutoff_PROBLEM(int64_t max_cache_size, int64_t min_cache_size, int64_t max_cache_misses, char cache_misses_out_fname[200]){
  ...
  while(cache_size >= min_cache_size){
    ...
    cache_misses = get_cache_misses_();
    if(cache_misses > max_cache_misses){
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"cache misses > max_cache_misses\n");fclose(fp);
      break;
    }
    fp=fopen(cache_misses_out_fname,"a");
    if(fp == NULL){
...
void explore(int64_t lo_cache_size, int64_t hi_cache_size, char cache_misses_out_fname[200], int cnt){
  ...
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"\t\t\texplore sizes %d %ld %ld\n", cnt,lo,hi);fclose(fp);
  reset_problem();
  reset_lru_queue(lo);
  set_test_num_evictions(0);
  solve_problem();
  int64_t m_lo = get_cache_misses_();
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"lo: size %ld misses %ld\n", lo, m_lo);fclose(fp);
  reset_problem();
  reset_lru_queue(hi);
  set_test_num_evictions(0);
  solve_problem();
  int64_t m_hi = get_cache_misses_();
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"hi: size %ld misses %ld\n", hi, m_hi);fclose(fp);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"\t\t\t\t\t\t\t\texplore misses %d %ld %ld\n", cnt,m_lo,m_hi);fclose(fp);
  if(m_lo == m_hi){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"\t\t\t\t\t\t\t\t%d misses %ld == %ld\n",cnt,m_lo,m_hi);fclose(fp);
    return;
  }
  if(lo == hi){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"\t\t\t\t\t\t\t\t%d sizes %ld == %ld\n",cnt,lo,hi);fclose(fp);
    return;
  }
  if(lo == hi-1){
    fp = fopen(cache_misses_out_fname, "a");
    ...
    fprintf(fp, "%ld,%ld\n", hi, m_hi);
    fclose(fp);
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"explore wrote (%ld,%ld)\n",hi,m_hi);fclose(fp);
    return;
  }
  int64_t mid = lo + (hi - lo) / 2;
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"\t\t\t\t\ttrying right %d %ld %ld %ld\n", cnt,lo,mid,hi);fclose(fp);
  explore(mid, hi, cache_misses_out_fname, cnt+1);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"trying left  %d %ld %ld %ld\n", cnt,lo,mid,hi);fclose(fp);
...
void explore_fixed_spot(int64_t cache_size, char cache_misses_out_fname[200]){
  ...
  fprintf(fp,"%ld,%ld\n",cache_size,misses);
  fclose(fp);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"explore fixed spot wrote (%ld,%ld)\n",cache_size,misses);fclose(fp);
}
...
int memo_batch_test(int argc, char ** argv){
    ...
    if(strcmp(argv[g], mbt_metric_type_parameter) == 0){
      if(g+1 < argc){
        ...
        if(strcmp(argv[g], csize_linear_search_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"csize_linear_search chosen\n");fclose(fp);
          metric_type = LINEAR_SEARCH;
        }
        if(strcmp(argv[g], csize_binary_search_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"csize_binary_search chosen\n");fclose(fp);
          metric_type = BINARY_SEARCH;
        }
        if(strcmp(argv[g], no_preemptive_halt_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"no_preemptive_halt chosen\n");fclose(fp);
          metric_type = NO_HALT;
        }
        if(strcmp(argv[g], solve_once_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"solve_once chosen\n");fclose(fp);
          metric_type = SOLVE_ONCE;
        }
        if(strcmp(argv[g], ratio_based_cutoff_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"ratio based cutoff parameter chosen\n");fclose(fp);
          metric_type = RATIO_BASED;
        }
        if(strcmp(argv[g], exact_cutoff_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"exact cutoff parameter chosen\n");fclose(fp);
          metric_type = EXACT_CUTOFF;
        }
        if(strcmp(argv[g], explore_sweet_spots_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"explore sweet spots parameter chosen\n");fclose(fp);
          metric_type = EXPLORE_SWEET_SPOTS;
        }
        if(strcmp(argv[g], explore_fixed_startpoints_parameter) == 0){
          fp=fopen(execution_trace_fname,"a");fprintf(fp,"explore fixed startpoints\n");fclose(fp);
  ...
  if( strcmp(problem_type, edit_distance_parameter)==0 ){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"edit_distance %s %ld\n", problem_type, cache_size);fclose(fp);
  ...
  if( strcmp(problem_type, kmp_parameter)==0 ){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"kmp %s %ld\n", problem_type, cache_size);fclose(fp);
  ...
  if( strcmp(problem_type, fibonacci_parameter)==0 ){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"fibonacci %s %ld\n", problem_type, cache_size);fclose(fp);
  ...
  if( strcmp(problem_type, arora_parameter)==0 ){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"arora %s %ld\n", problem_type, cache_size);fclose(fp);
  ...
  if( strcmp(problem_type, test_parameter)==0 ){
    fp=fopen(execution_trace_fname,"a");fprintf(fp,"test\n");fclose(fp);
    test_operation_sequence(sequence_fname);
    return 0;
  }
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"initializing problem\n");fclose(fp);
  initialize_problem(argc, argv);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"done initializing problem\n");fclose(fp);
  fp=fopen(num_evictions_fname, "a");
  if(fp == NULL){
  ...
  fprintf(fp, ",%s\n", instance_fname);
  fclose(fp);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"commence trials\n");fclose(fp);
  int64_t metric = -1;
  switch(metric_type){
    case LINEAR_SEARCH:
      metric = characteristic_cache_size_linear_search(cache_size, cache_misses_out_fname);
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"characteristic cache size (linear search) for %s = %ld\n",instance_fname, metric);fclose(fp);
      break;
    case BINARY_SEARCH:
      metric = characteristic_cache_size_binary_search(cache_size, cache_misses_out_fname);
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"characteristic cache size (binary search) for %s = %ld\n", instance_fname, metric);fclose(fp);
      break;
    case NO_HALT:
      metric = non_preemptive_linear_search(cache_size, cache_misses_out_fname);
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"non-preemptive linear search for %s = %ld\n", instance_fname, metric);fclose(fp);
      break;
    case SOLVE_ONCE:
      metric = solve_once(cache_size, cache_misses_out_fname);
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"solve once for %s = %ld\n", instance_fname, metric);fclose(fp);
      break;
    case RATIO_BASED:
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"case RATIO_BASED\n");fclose(fp);
      ...
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"ratio based for %s = %ld\n", instance_fname, metric);fclose(fp);
      break;
    case EXACT_CUTOFF:
      ...
      fp=fopen(execution_trace_fname,"a");fprintf(fp,"exact cutoff for %s = %ld\n",instance_fname, metric);fclose(fp);
      break;
...
void test_operation_sequence_with_feedback(operation_sequence * os){
  ...
  for(g = 0; g < os->num_operations; g++){
    ...
    switch(os->operation[g]){
      case 0:
        ...
        if(x == NULL){ 
          if(os->expected_result[g]==1){
            fp=fopen(execution_trace_fname,"a");fprintf(fp,"error, read didn't find but should have\n");fclose(fp);
          ...
          if(os->expected_result[g]==0){
            fp=fopen(execution_trace_fname,"a");fprintf(fp,"error, read found but should not have\n");fclose(fp);
            view_hashtable_long_int();
            exit(1);
            num_errors++;
          }
      ...
      case 1:
        ...
        if(x == NULL){
          if(os->expected_result[g]==1){
            fp=fopen(execution_trace_fname,"a");fprintf(fp,"error\n");fclose(fp);
            view_hashtable_long_int();
            exit(1);
            num_errors++;
          }
  ...
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"num_errors = %ld\n",num_errors);fclose(fp);
}
...
void test_operation_sequence(char sequence_fname[200]){
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"test_operation_sequence\n");fclose(fp);
  operation_sequence * os = read_operation_sequence(sequence_fname);
  fp=fopen(execution_trace_fname,"a");fprintf(fp,"got operation sequence\n");fclose(fp);
